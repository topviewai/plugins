...
Before submit, read [`references/generation-planning.md`](references/generation-planning.md) (capabilities before task counts; universal duration splits). If any `inputs` come from local files or FFmpeg artifacts, read [`references/local-media-bridge.md`](references/local-media-bridge.md) first and finish upload validation before submit.
