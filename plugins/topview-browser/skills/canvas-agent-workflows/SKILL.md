...
2. Make that Canvas visible **before** any generation submit or creative-surface action. Open it exactly once with `open_topview_canvas` (or the single workspace-specific alternative required by `$operate-topview-canvas`), then navigate the returned `webLink` with the host Browser when available. If no host Browser is available, provide the exact clickable link as the required handoff.
...
→ show that Canvas in the host Browser (or complete the clickable-link handoff)
