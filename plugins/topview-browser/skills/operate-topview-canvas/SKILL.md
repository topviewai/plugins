...
Use the plugin's `topview-browser` MCP server as the Canvas data and action boundary. It is the same remote MCP connection used by other Topview skills.
...
9. **Browser-native Canvas opening**: call exactly one of `open_topview_canvas` or `open_topview_canvas_workspace`; both validate access and return the authorized public `webLink` without iframe resources, embed tickets, OAuth tokens, or credentials in the URL. If the requested workspace is known, use only `open_topview_canvas_workspace` (currently `workspace="canvas"`); otherwise use only `open_topview_canvas`. Never call both for the same request, never assemble or guess a Canvas URL, and never offer a `ui://…` resource URI as a browser link.
10. **Prefer the host Browser**: after the MCP open result, use an available host Browser/navigation capability to open `webLink` automatically and keep the Canvas visible in the host's browser pane or right-side panel. Do not claim a fixed pane placement the host cannot guarantee. If Browser access is unavailable, disabled, or awaiting user approval, return `webLink` verbatim as a clickable Markdown fallback and state the specific boundary. A second Topview web login is acceptable; never request or relay passwords, cookies, OAuth codes, or tokens.
11. **Canvas-first creation**: image, video, music, whiteboard, `world_3d`, and `director_stage` creation must target a resolved/created Canvas that is already visible in the host Browser or has been handed off through the clickable fallback. Reuse an already-visible matching Canvas. Keep the Canvas visible so the user can participate manually throughout the workflow.
...
1. When the user asks to see the Canvas, open it exactly once: use only `open_topview_canvas_workspace` when the requested workspace is known, otherwise use only `open_topview_canvas`. Then navigate the returned `webLink` with the host Browser when available; otherwise present that exact link as the fallback. Do not call both MCP open tools. If the Canvas is already open and the user only asks for data mutations, do not reopen it.
...
8. The Browser plugin intentionally does not expose iframe-only helpers such as `focus_topview_canvas_nodes` or `open_topview_canvas_node_tool`. Keep using Canvas data tools for mutations and let the user inspect or finish UI-only forms in the open web Canvas. `download_topview_canvas_nodes` remains usable when the user explicitly requests downloads because it returns artifacts rather than rendering the Canvas.
...
- In the browser Canvas, persisted TextNode `backgroundColor` and `backgroundOpacity` are rendered. If a populated note is still not visible, keep the content, restore the safe visual payload, and ask the user to inspect the card in the open Canvas rather than recreating it blindly.
...
2. Use `commandId`, `sceneNumber`, `sceneSummary`, `sceneText`, optional `referenceNodeIds` (at most 32 live Asset nodes), optional `duration` (integer 4…30), optional `aspectRatio`, and layout. Do **not** send `environmentText`, `plotText`, `storyboardPrompt`, or `videoPrompt`. Do **not** send `expectedRevision`.
...
- The Browser plugin opens the normal web Canvas, so the built-in Canvas Agent, node **Add to chat**, Timeline UI, and manual **Add to Timeline** actions remain available according to the user's web account permissions.
- If the browser Canvas fails to load, report the observed browser error. Do not loop MCP open calls or guess another URL.
- Browser links come only from an authorized MCP open result or `get_topview_canvas_web_link`. Never derive a link from an iframe resource, embed bootstrap route, or Canvas ID.
